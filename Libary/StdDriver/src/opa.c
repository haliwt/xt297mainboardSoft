...
void OPA_SetRunMode(uint8_t OPAn, uint8_t OpaMode)
{
	if(OPA0 == OPAn)
	{
		OP0CON0 &= ~(OPA_OP0CON0_OP0FIL_Msk);
		OP0CON0 |= (OpaMode << OPA_OP0CON0_OP0FIL_Pos);
	}
	if(OPA1 == OPAn)
	{
		OP1CON0 &= ~(OPA_OP1CON0_OP1FIL_Msk);
		OP1CON0 |= (OpaMode << OPA_OP1CON0_OP1FIL_Pos);	
	}
}
...
void OPA_ConfigPositive(uint8_t OPAn, uint8_t PositiveSource)
{
	if(OPA0 == OPAn)
	{
		OP0CON0 &= ~(OPA_OP0CON0_OP0PS_Msk);
		OP0CON0 |= (PositiveSource << OPA_OP0CON0_OP0PS_Pos);
	}
	if(OPA1 == OPAn)
	{
		OP1CON0 &= ~(OPA_OP1CON0_OP1PS_Msk);
		OP1CON0 |= (PositiveSource << OPA_OP1CON0_OP1PS_Pos);	
	}
}
...
void OPA_ConfigNegative(uint8_t OPAn, uint8_t NegativeSource)
{
	if(OPA0 == OPAn)
	{
		OP0CON0 &= ~(OPA_OP0CON0_OP0NS_Msk);
		OP0CON0 |= (NegativeSource << OPA_OP0CON0_OP0NS_Pos);
	}
	if(OPA1 == OPAn)
	{
		OP1CON0 &= ~(OPA_OP1CON0_OP1NS_Msk);
		OP1CON0 |= (NegativeSource << OPA_OP1CON0_OP1NS_Pos);	
	}
